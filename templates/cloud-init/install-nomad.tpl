...
write_files:
- content: ""
  path: ${nomad_path}/bin/.cloudcreate
  owner: root:root
  permissions: '0644'
- content: ""
  path: ${nomad_path}/config/.cloudcreate
  owner: root:root
  permissions: '0644'
- content: ""
  path: ${nomad_path}/data/.cloudcreate
  owner: root:root
  permissions: '0644'
- content: |
    #!/bin/bash
...
- content: |
    ...
    arm*)
      ...
      ;;
    *)
      log_error "CPU architecture $cpu_arch is not a supported by Nomad."
      exit 1
      ;;
    esac

    url="https://releases.hashicorp.com/nomad/${NOMAD_VERSION}/nomad_${NOMAD_VERSION}_linux_$${binary_arch}.zip"
    download_path="/tmp/nomad_${NOMAD_VERSION}_linux_$${binary_arch}.zip"
    bin_dir="${nomad_path}/bin"
    nomad_dest_path="$bin_dir/nomad"
    ...
    symlink_path="$SYSTEM_BIN_DIR/nomad"
    if [[ -f "$symlink_path" ]]; then
      exit 0;
    else
      echo "Adding symlink to $nomad_dest_path in $symlink_path"
      sudo ln -s "$nomad_dest_path" "$symlink_path"
    fi
  path: /opt/bootstrap/install-nomad
  owner: root:root
  permissions: '0755'
users:
- name: ${username}
...
- jq
runcmd:
- /opt/bootstrap/install-nomad
- chown -R ${username}:${username} ${nomad_path}
- ${nomad_path}/bin/run-nomad --server --num-servers ${count}
