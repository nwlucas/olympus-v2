...
write_files:
- content: ""
  path: ${consul_path}/bin/.cloudcreate
  owner: root:root
  permissions: '0644'
- content: ""
  path: ${consul_path}/config/.cloudcreate
  owner: root:root
  permissions: '0644'
- content: ""
  path: ${consul_path}/data/.cloudcreate
  owner: root:root
  permissions: '0644'
- content: ""
  path: ${consul_path}/tls/ca/.cloudcreate
  owner: root:root
  permissions: '0644'
- content: |
    #!/bin/bash
    ...
    run "$@"
  path: ${consul_path}/bin/run-consul
  owner: root:root
  permissions: '0755'
- content: |
    #!/bin/bash -eux
    ...
    *)
    ...
    url="https://releases.hashicorp.com/consul/${CONSUL_VERSION}/consul_${CONSUL_VERSION}_linux_$${binary_arch}.zip"
    download_path="/tmp/consul_${CONSUL_VERSION}_linux_$${binary_arch}.zip"
    bin_dir="${consul_path}/bin"
    consul_dest_path="$bin_dir/consul"
    ...
    symlink_path="$SYSTEM_BIN_DIR/consul"
  ...
  path: /opt/bootstrap/install-consul
  owner: root:root
  permissions: '0755'
users:
- name: ${username}
...
- jq
runcmd:
- /opt/bootstrap/install-consul
- chown -R ${username}:${username} ${consul_path}
- ${consul_path}/bin/run-consul --server --cluster-size ${count} --cluster-api-token ${DIGITALOCEAN_API_TOKEN} --cluster-tag-name ${cluster_tag}
